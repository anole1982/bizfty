...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="ENABLE-RESOURCE-PARAGRAPH" select="true()"/>
    <xsl:param name="ENABLE-OPERATIONS-PARAGRAPH" select="false()"/>
    <xsl:param name="ENABLE-METHODS-PARAGRAPH" select="true()"/>
    ...
    <xsl:variable name="PORT-TYPE-TEXT">服务</xsl:variable>
    <xsl:variable name="IFACE-TEXT">接口</xsl:variable>
    <xsl:variable name="SOURCE-CODE-TEXT">源码</xsl:variable>
    ...
    <xsl:variable name="global.xsd-name"
                  select="
      ($consolidated-wsdl/*[local-name() = 'types']//xsd:import[@schemaLocation] |
      $consolidated-wsdl/*[local-name() = 'types']//xsd:include[@schemaLocation])[1]/@schemaLocation"/>
    <xsl:variable name="consolidated-xsd"
                  select="
(document($global.xsd-name)/xsd:schema/xsd:*|/*/*[local-name() = 'types']/xsd:schema/xsd:*)[local-name() = 'complexType' or local-name() = 'element' or local-name() = 'simpleType'] |
(document($global.xsd-name)/xsd:schema/xsd:*|/*/*[local-name() = 'grammars']/xsd:schema/xsd:*)[local-name() = 'complexType' or local-name() = 'element' or local-name() = 'simpleType'] "/>
    ...
    <xsl:variable name="global.resources-base" select="$consolidated-wsdl/wadl:resources/@base"/>
    ...
    <xsl:template match="ws:definitions | ws2:description | wadl:application" mode="html-title.render">
        <xsl:choose>
            <xsl:when test="$global.service-name">
                <xsl:value-of select="concat('服务: ', $global.service-name)"/>
            </xsl:when>
            ...
            <xsl:when test="$global.resources-base">
                <xsl:variable name="lastIndex">
                    <xsl:call-template name="last-index-of">
                        <xsl:with-param name="string" select="$global.resources-base"/>
                        <xsl:with-param name="substring" select="'/'"/>
                    </xsl:call-template>
                </xsl:variable>
                <xsl:value-of select="concat('资源: ', substring($global.resources-base,$lastIndex+1,string-length($global.resources-base)))"/>
            </xsl:when>
    ...
    <xsl:template match="wadl:resource" mode="service-root">
        <xsl:variable name="path" select="concat($global.resources-base,@path)"/>
        <div class="label">方法:</div>
        <div class="value">
            <xsl:text>
            </xsl:text>
            <ol style="line-height: 180%;">
                <xsl:apply-templates select="wadl:resource" mode="service">
    ...
    <xsl:template match="wadl:resource" mode="service">
        <xsl:param name="path"/>
        <li>
            <big>
                <i>
            ...
            <xsl:if test="$ENABLE-LINK">
                <xsl:if test="$ENABLE-METHODS-PARAGRAPH">
                    <a class="local" href="{concat('#', $OPERATIONS-PREFIX, generate-id(.))}">详情</a>
                </xsl:if>
                <xsl:call-template name="render.source-code-link"/>
            </xsl:if>
    ...
    <xsl:template match="ws:port" mode="service">
        ...
        <xsl:variable name="port-type-documentation"
                      select="$consolidated-wsdl/ws:portType[@name = $port-type-name]/ws:documentation"/>

        <div class="label">服务描述:</div>
        <div class="value">
            <xsl:value-of select="$port-type-documentation"/>
        </div>
        <h3>Port <b>
                <xsl:value-of select="@name"/>
            </b>
    ...
    <xsl:template match="wadl:resource" mode="operations-root">
        <div>
            <xsl:if test="position() != last()">
                <xsl:attribute name="class">port</xsl:attribute>
            </xsl:if>
            <xsl:if test="$ENABLE-PORTTYPE-NAME">
                <h3>
                    <a name="{concat($PORT-PREFIX, generate-id(.))}">
                        ...
                        <b> 
                            <xsl:value-of select="concat($global.resources-base,@path)"/> 
                        </b>
                    </a>
                    <xsl:call-template name="render.source-code-link"/>
                </h3>
            ...
            <ol>
                <xsl:apply-templates select="wadl:resource" mode="operations">
                    <xsl:sort select="@path"/>
                </xsl:apply-templates>
            </ol>
        </div>
    </xsl:template>
    <xsl:template match="wadl:resource" mode="operations">
        <li>
            <xsl:if test="position() != last()">
                <xsl:attribute name="class">operation</xsl:attribute>
            </xsl:if>
            ...
            <xsl:if test="$ENABLE-DESCRIPTION and string-length(wadl:documentation) &gt; 0">
                <div class="label">方法描述:</div>
                <div class="value">
                    <xsl:value-of select="ws:documentation" disable-output-escaping="yes"/>
                </div>
            </xsl:if>
            ...
            <xsl:apply-templates select="wadl:method/wadl:request" mode="operations.message">
                <xsl:with-param name="binding-data" select="wadl:method/wadl:request"/>
            </xsl:apply-templates>
            <xsl:apply-templates select="wadl:method/wadl:response" mode="operations.message">
                <xsl:with-param name="binding-data" select="wadl:method/wadl:response"/>
            </xsl:apply-templates>
        </li>
    </xsl:template>

    <xsl:template match="wadl:request|wadl:response" mode="operations.message">
        <xsl:param name="binding-data"/>
        <xsl:value-of select="$binding-data/wadl:representation/@mediaType"/>
        <xsl:if test="$ENABLE-INOUTFAULT">
            <div class="label">
                ...
                <xsl:if test="$type =  'Fault'">
                    异常:
                </xsl:if>
            </div>
            <xsl:variable name="msg-element" select="wadl:representation"/>
            <xsl:apply-templates select="wadl:representation" mode="operations.message"/>
        </xsl:if>
    </xsl:template>
    
    <xsl:template match="wadl:representation" mode="operations.message">
    ...
    <xsl:template match="ws:input|ws:output|ws:fault" mode="operations.message">
        ...
        <xsl:if test="$ENABLE-INOUTFAULT">
            <div class="label">
                <xsl:variable name="type" select="concat(translate(substring(local-name(.), 1, 1), 'abcdefghijklmnoprstuvwxyz', 'ABCDEFGHIJKLMNOPRSTUVWXYZ'), substring(local-name(.), 2))" />
    ...
    <xsl:template name="body.render">
        <div>
            <xsl:value-of select="$consolidated-wsdl"/>
        </div>
        <body id="operations">
            <div id="outer_box">
    ...
    <xsl:template name="content.render">
        <div id="content">
            ...
            <xsl:if test="$ENABLE-RESOURCE-PARAGRAPH">
                <xsl:call-template name="resource.render"/>
            </xsl:if>
            <xsl:if test="$ENABLE-OPERATIONS-PARAGRAPH">
                <xsl:call-template name="operations.render"/>
            </xsl:if>
            <xsl:if test="$ENABLE-METHODS-PARAGRAPH">
                <xsl:call-template name="methods.render"/>
            </xsl:if>
    ...
    <!--
    ==================================================================
            Rendering: WADL service information
    ==================================================================
    -->

    <xsl:template name="resource.render">
        <div class="page">
            ...
            <!--绘制清单列表-->
            <xsl:apply-templates select="$consolidated-wsdl/wadl:resources/wadl:resource" mode="service-root"/>
        </div>
    </xsl:template>
    ...
    <!--
    ==================================================================
            Rendering: WADL operations - detail
    ==================================================================
    -->

    <xsl:template name="methods.render">
        <div class="page">
            <a class="target" name="page.operations">
                <h2>方法清单</h2>
            </a>
            <ul>
                <xsl:apply-templates select="$consolidated-wsdl/wadl:resources/wadl:resource" mode="operations-root">
                    <xsl:sort select="@path"/>
                </xsl:apply-templates>
            </ul>
        </div>
    ...
    <xsl:template name="last-index-of">
        ...
        <xsl:choose>
            <xsl:when test="contains($string,$substring)">
                ...
                <xsl:value-of select="string-length($string) - (string-length(substring-before($rstring,$rsubstring)) + string-length($rsubstring)) + 1"/>
            </xsl:when>
            <xsl:otherwise>
